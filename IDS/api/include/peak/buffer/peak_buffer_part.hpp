...
inline BufferPart::~BufferPart()
{
    (void)PEAK_C_ABI_PREFIX PEAK_BufferPart_Destruct(m_backendHandle);
}
...
{
    return QueryNumericFromCInterfaceFunction<size_t>(
        [&](size_t* height) { return PEAK_C_ABI_PREFIX PEAK_BufferPart_GetHeight(m_backendHandle, height); });
}
