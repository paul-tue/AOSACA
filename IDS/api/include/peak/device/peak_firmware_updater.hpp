...
public:
     ...
     *
     * \note This function requires the given \p device to be openable.
     *
     * \since 1.0
     *
     * \throws InvalidArgumentException One of the submitted arguments is outside the valid range or is not supported.
     ...
     * \throws IOException A communication error has occured, e.g. when the device gets unexpectedly disconnected during the update process.
     * \throws InternalErrorException An internal error has occurred.
     */
    ...
    const std::string& gufPath, const std::shared_ptr<DeviceDescriptor>& device) const
{
    if (device == nullptr)
    {
        throw InvalidArgumentException("The given device is a nullptr!");
    }
    ...
    Timeout deviceResetDiscoveryTimeout /* = Timeout{ 60000ms }*/)
{
    ...
    {
        throw InvalidArgumentException("The given updateInformation is a nullptr!");
    }
