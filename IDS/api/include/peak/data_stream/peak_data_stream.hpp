...
public:
     ...
     * timeout is exceeded.
     *
     * Additionally the blocking wait can be aborted by calling \ref KillWait.
     *
     * \note After the buffer has been received it has to be requeued using
     *       \ref QueueBuffer once the buffer data has been copied or processed.
     ...
     *                       Use \ref DataStream::INFINITE_NUMBER to wait indefinitely.
     *
    ...
    /*!
     * \brief Tells the data stream to kill a wait for a finished buffer.
     *
     * Kills one call to \ref WaitForFinishedBuffer or queues the kill request
     * to be applied to the next call to \ref WaitForFinishedBuffer immediately.
     *
     * \since 1.0
     *
     * \throws InternalErrorException An internal error has occurred.
